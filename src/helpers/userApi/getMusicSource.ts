...
import { DEV_URL_PREFIX, BACKUP_URL_PREFIX } from './third_party_url'
...
const handleBackupFetch = (backupUrl: string, options: any, type: string, fakeAudioMp3Uri: string) => {
    ...
    return fetchWithTimeout(url, options, 5000)
        ...
        .then((body: any) => {
            if (!body.data || (typeof body.data === 'string' && body.data.includes('error'))) {
                ...
                return handleBackupFetch(backupUrl, options, type, fakeAudioMp3Uri);
            }
